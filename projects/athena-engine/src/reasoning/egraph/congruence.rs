...
pub fn application_congruence_candidates(
    ...
    let apps: Vec<TermId> = {
        let mut terms: Vec<TermId> = graph
            .known_terms()
            .into_iter()
            .filter(|&t| matches!(store.get(t), Some(TermNode::Application { .. })))
            .collect();
    ...
    for i in 0..apps.len() {
        for j in (i + 1)..apps.len() {
            ...
            let Some(left_class) = graph.class_of_term(left) else {
                continue;
            };
            let Some(right_class) = graph.class_of_term(right) else {
                continue;
            };
            if graph.find(left_class) == graph.find(right_class) {
                continue;
            }
            out.push(CandidateEquivalence {
                left_term: left,
                right_term: right,
                left_class,
                right_class,
        ...
        (
            ...
            head_l == head_r
                ...
                && args_l
                    .iter()
                    .zip(args_r.iter())
                    .all(|(a, b)| exact_uf.find(*a) == exact_uf.find(*b))
        }
        _ => false,
    }
}
...
pub fn admit_application_congruence_candidates(
    ...
    candidates
        .iter()
        .map(|c| admit_application_congruence(store, semantic, c.left_term, c.right_term, policy))
        .collect()
}
