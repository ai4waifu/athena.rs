...
pub fn make_monic(mut coeffs: Vec<Rational>) -> Result<Vec<Rational>> {
    ...
    let lead = coeffs.last().map(clone_rational).ok_or_else(|| ext_err("empty_modulus"))?;
    if lead.is_zero() {
...
pub fn canonical_nf_coords(mut coords: Vec<Rational>, degree: u32) -> Result<Vec<Rational>> {
    ...
    resize_rationals(&mut coords, n, &Rational::zero());
    Ok(coords)
}
...
pub fn embed_base_coords(base_coords: &[Rational], absolute_degree: u32, base_degree: u32) -> Result<Vec<Rational>> {
    ...
    let mut out = {
        let mut __v = Vec::new();
        resize_rationals(&mut __v, absolute_degree as usize, &Rational::zero());
        __v
    };
    for (dst, src) in out.iter_mut().zip(base_coords.iter()) {
        *dst = clone_rational(src);
    }
    Ok(out)
}
...
pub fn relative_modulus_from_rational(coeffs: &[Rational], base_degree: u32) -> Result<Vec<Vec<Rational>>> {
    ...
    Ok(monic
        ...
        .map(|c| {
            let mut block = {
                let mut __v = Vec::new();
                resize_rationals(&mut __v, bd, &Rational::zero());
                __v
            };
            block[0] = c;
            block
...
pub fn mul_relative_nf_coords(
    ...
    let mut prod = {
        let mut __out = Vec::with_capacity(2 * rd);
        for _ in 0..(2 * rd) {
            let mut __v = Vec::new();
            resize_rationals(&mut __v, bd, &Rational::zero());
            __out.push(__v);
    ...
    let dg = g.len() - 1;
    while prod.len() > dg {
        let lead = prod.last().map(|v| clone_rationals(v)).unwrap_or_else(|| {
            let mut __v = Vec::new();
            resize_rationals(&mut __v, bd, &Rational::zero());
            __v
        });
...
pub fn inv_relative_nf_coords(
    ...
    let mut one = {
        let mut __v = Vec::new();
        resize_rationals(&mut __v, abs, &Rational::zero());
        __v
    };
    one[0] = Rational::one();
...
fn div_rem_z(a: &[Integer], b: &[Integer]) -> (Vec<Integer>, Vec<Integer>) {
    ...
    let mut quot = {
        let mut __v = Vec::new();
        resize_integers(&mut __v, rem.len() - bb.len() + 1, &Integer::zero());
        __v
    };
...
fn poly_mul_q(a: &[Rational], b: &[Rational]) -> Vec<Rational> {
    ...
    let mut out = {
        let mut __v = Vec::new();
        resize_rationals(&mut __v, a.len() + b.len() - 1, &Rational::zero());
        __v
    };
...
fn poly_mod_q(a: &[Rational], m: &[Rational]) -> Vec<Rational> {
    ...
    resize_rationals(&mut rem, dm, &Rational::zero());
    rem
}
...
fn poly_egcd_q(a: &[Rational], b: &[Rational]) -> Result<(Vec<Rational>, Vec<Rational>, Vec<Rational>)> {
    ...
    let lead = r0.last().map(clone_rational).unwrap_or_else(Rational::one);
    if lead.is_zero() {
...
fn poly_div_rem_q(a: &[Rational], b: &[Rational]) -> Result<(Vec<Rational>, Vec<Rational>)> {
    ...
    let mut quot = {
        let mut __v = Vec::new();
        resize_rationals(&mut __v, rem.len() - b.len() + 1, &Rational::zero());
        __v
    };
    while rem.len() >= b.len() {
        let dr = rem.len() - 1;
...
fn shortest_relation(powers: &[Vec<Rational>]) -> Result<Option<Vec<Rational>>> {
    ...
    let mut mat = {
        let mut __out = Vec::with_capacity(rows);
        for _ in 0..(rows) {
            let mut __v = Vec::new();
            resize_rationals(&mut __v, cols, &Rational::zero());
            __out.push(__v);
        }
        __out
    };
    ...
    for row in 0..rows {
        while col < cols {
            ...
            let pv = clone_rational(&mat[row][col]);
            for c in col..cols {
            ...
            for r in 0..rows {
                ...
                let f = clone_rational(&mat[r][col]);
                if f.is_zero() {
                    continue;
                }
                for c in col..cols {
...
fn flatten_blocks(blocks: &[Vec<Rational>]) -> Vec<Rational> {
    blocks.iter().flatten().map(clone_rational).collect()
}

fn solve_linear_q(columns: &[Vec<Rational>], target: &[Rational]) -> Result<Vec<Rational>> {
    let n = columns.len();
    let mut mat = {
        let mut __out = Vec::with_capacity(n);
        for _ in 0..(n) {
            let mut __v = Vec::new();
            resize_rationals(&mut __v, n + 1, &Rational::zero());
            __out.push(__v);
        }
        __out
    };
    ...
    for i in 0..n {
        mat[i][n] = clone_rational(&target[i]);
    }
    for col in 0..n {
        let mut pivot = None;
        for r in col..n {
        ...
        let pv = clone_rational(&mat[col][col]);
        for c in col..=n {
    ...
    Ok((0..n).map(|i| clone_rational(&mat[i][n])).collect())
}
