...
pub fn normalize_relational_app(arena: &TermArena, root: TermId, ops: &RelationalOperators) -> Result<Constraint, Diagnostic> {
    let (kind, span) = arena_node(arena, root)?;
    let TermKind::App { op, args } = kind
    else {
...
fn arena_node(arena: &TermArena, id: TermId) -> Result<(&TermKind, SourceSpan), Diagnostic> {
