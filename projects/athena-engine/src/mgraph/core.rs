//! 理论层完整规格见 [`super::theory`] 模块文档（**非运行时**）。
//!
//! 实现层：`MGraphCore { scope_index, relation_index }` + `admit` / `close`。
//! 理论层概念（根源宇宙 `𝒰`、纤维化、内世界子范畴）**不得**出现在本模块的类型中。

use super::{
    claim::VerifiedClaim,
    refs::{RelationRef, ScopeRef, ScopeRelationKind},
    relation_index::{RelationIndex, RelationRecord},
...
impl<'a> MGraphView<'a> {
    ...
    /// Scope 边。
    pub fn scope_edges(&self) -> &[super::scope_index::ScopeEdge] {
