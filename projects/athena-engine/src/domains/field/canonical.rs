...
pub fn apply_field_automorphism(table: &FieldTable, maps: &MapTable, aut: AutomorphismId, element: &FieldElement) -> Result<FieldElement> {
    ...
    match &element.repr {
        FieldElementRepr::ExtensionCoords { coords } => {
            let next = crate::domains::algebra::apply_frobenius_coords(table, extension.id, coords, power)?;
...
pub fn apply_field_embedding(table: &FieldTable, maps: &MapTable, map_id: AlgebraMapId, element: &FieldElement) -> Result<FieldElement> {
    ...
    if !matches!(map.kind, crate::domains::algebra::AlgebraMapKind::FieldEmbedding) {
    ...
    match &element.repr {
        FieldElementRepr::Rational { value } => {
            ...
            let inv = crate::domains::number_theory::mod_inverse(&denom, &modulus)?;
...
pub fn inv_field_element(table: &FieldTable, element: &FieldElement) -> Result<FieldElement> {
    match &element.repr {
        ...
        FieldElementRepr::PrimeFieldResidue { value } => {
            ...
            let inv = crate::domains::number_theory::mod_inverse(value, &modulus)?;
...
fn field_endpoints(map: &crate::domains::algebra::AlgebraMap) -> Result<(FieldId, FieldId)> {
