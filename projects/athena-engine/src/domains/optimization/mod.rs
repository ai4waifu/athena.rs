...
//!
//! 优化不是 [`crate::domains::solve`] 的别名。本模块拥有问题身份、变量/约束/目标、
//! 可行集、结果状态与证书合同。调度复用 [`crate::reasoning::solver`]，数值与矩阵复用
//! [`crate::domains::linear_algebra`] / `athena-numeric`。
//!
//! 算法（simplex / branch-and-bound / SQP 等）在语义地基稳定前仅标引导实现。
//! 禁止新增 `athena-optimization` 微 crate。
//!
//! 与 [`crate::domains::solve::Constraint`] 不同：本模块的 [`Constraint`] 是优化可行域约束，
