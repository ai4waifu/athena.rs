//! KernelIR 编译器 — `TermArena` 子树 → [`ExecUnit`]（Living `25` L2）。
//!
...
use super::{
    ids,
    kernel::{ExecUnit, HandlerId, Instr},
...
fn lower_set_stmt(vm: &mut Vm<'_>, name: &str, args: Vec<TermId>, code: &mut Vec<Instr>) {
    ...
    let lhs_symbol = match vm.session.arena.get(lhs) {
        Some(TermKind::Atom(AtomKind::Symbol(sym))) => Some(*sym),
        _ => None,
    };
    if name == "Set" {
    ...
    else if let Some(TermKind::App { op, .. }) = vm.session.arena.get(lhs) {
        // `f[x_] := rhs`
