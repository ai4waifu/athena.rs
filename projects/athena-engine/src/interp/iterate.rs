...
use super::{
    Outcome,
    arith::number_of,
    builtin::{non_boolean_condition_diagnostic, term_summary},
    vm::Vm,
};
...
pub(crate) fn table(vm: &mut Vm<'_>, args: &[TermId]) -> (Outcome, Option<Vec<TermId>>) {
    ...
    for value in values {
        let body = match var {
            Some(sym) => super::pattern::substitute_symbol(vm, args[0], sym, value),
            None => args[0],
        };
    ...
    (
        Outcome { term, kind: super::EvalKind::Value, status: athena_types::ComputationStatus::Exact, diagnostics: diags },
        Some(out),
    )
}
...
fn expand_iterator(vm: &mut Vm<'_>, spec: TermId) -> Option<(Option<athena_types::SymbolId>, Vec<TermId>)> {
    let items = match vm.session.arena.get(spec) {
        Some(TermKind::List(_)) => vm.app_args(spec)?,
        _ => return None,
    };
    match items.as_slice() {
        [var, n] => {
            ...
            Some((Some(sym), super::builtin::range_ints(vm, 1, n, 1)?))
        }
        [var, a, b] => {
            let sym = symbol_id(vm, *var)?;
            let a = number_of(vm, *a)?.as_exact_integer()?;
            let b = number_of(vm, *b)?.as_exact_integer()?;
            Some((Some(sym), super::builtin::range_ints(vm, a, b, 1)?))
        }
        [var, a, b, step] => {
            let sym = symbol_id(vm, *var)?;
            let a = number_of(vm, *a)?.as_exact_integer()?;
            ...
            Some((Some(sym), super::builtin::range_ints(vm, a, b, step)?))
        }
        [n] => {
            let n = number_of(vm, *n)?.as_exact_integer()?;
            Some((None, super::builtin::range_ints(vm, 1, n, 1)?))
        }
        _ => None,
    }
}
...
fn symbol_id(vm: &Vm<'_>, id: TermId) -> Option<athena_types::SymbolId> {
    match vm.session.arena.get(id) {
        Some(TermKind::Atom(AtomKind::Symbol(s))) => Some(*s),
        _ => None,
    }
}
...
pub(crate) fn sum_product(vm: &mut Vm<'_>, head: &str, args: &[TermId]) -> Outcome {
    match args {
        [only] if head == "Sum" => {
            ...
            let summed = super::lin::array_sum(vm, o.term);
            if summed.has_error() {
...
pub(crate) fn h_match_q(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    ...
    let matched = super::pattern::pattern_matches(vm, expr_o.term, args[1]);
...
pub(crate) fn h_cases(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    ...
    let Some(items) = (match vm.session.arena.get(list_o.term) {
        Some(TermKind::List(_)) => vm.app_args(list_o.term),
        _ => None,
    })
    else {
    ...
    let out: Vec<TermId> = items.into_iter().filter(|item| super::pattern::pattern_matches(vm, *item, args[1])).collect();
    Outcome {
        term: vm.push_list(out),
        kind: super::EvalKind::Value,
        status: athena_types::ComputationStatus::Exact,
