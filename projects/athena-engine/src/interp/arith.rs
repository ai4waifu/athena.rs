...
pub(crate) fn push_number(vm: &mut Vm<'_>, n: Number) -> TermId {
    ...
    vm.session.arena.push(TermKind::Atom(AtomKind::Number(n)), span)
}

/// 读 arena 数字引用。
pub(crate) fn number_of<'a>(vm: &'a Vm<'_>, id: TermId) -> Option<&'a Number> {
    match vm.session.arena.get(id) {
        Some(TermKind::Atom(AtomKind::Number(n))) => Some(n),
        _ => None,
    }
}
...
fn times_outcome(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    ...
    for a in args {
        ...
        else if matches!(vm.session.arena.get(*a), Some(TermKind::List(_))) {
            lists.push(*a);
        }
        else {
            other = true;
    ...
    if !other && lists.len() == 1 && !scalars.is_empty() && scalars.len() + 1 == args.len() {
        ...
        return super::lin::dot_binop(vm, "DotTimes", scale, lists[0], super::lin::DotOpKind::Times);
    }
    if !other && lists.len() == 2 && scalars.is_empty() && args.len() == 2 {
        let am = super::lin::term_to_rational_matrix(vm, lists[0]);
        let bm = super::lin::term_to_rational_matrix(vm, lists[1]);
        if let (Some(am), Some(bm)) = (am, bm) {
            let echo = vm.push_app("Times", args.to_vec());
            return match crate::linear_algebra::matmul(&am, &bm) {
                Ok(m) => match super::lin::matrix_to_nested_list(vm, &m) {
...
pub(crate) fn h_dot_times(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    super::lin::dot_binop(vm, "DotTimes", args[0], args[1], super::lin::DotOpKind::Times)
}

pub(crate) fn h_dot_divide(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    super::lin::dot_binop(vm, "DotDivide", args[0], args[1], super::lin::DotOpKind::Divide)
}

pub(crate) fn h_dot_power(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    super::lin::dot_binop(vm, "DotPower", args[0], args[1], super::lin::DotOpKind::Power)
}
...
pub(crate) fn h_mldivide(vm: &mut Vm<'_>, operands: &[TermId]) -> Outcome {
    ...
    super::lin::mldivide(vm, &name, a.term, b.term, echo).with_diagnostics(diags)
}
