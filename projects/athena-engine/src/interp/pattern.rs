    ...
    match name.as_str() {
        "Integer" => match vm.shape(expr) {
            Some(Shape::Number) => match vm.session.arena.get(expr) {
                Some(athena_ir::TermKind::Atom(athena_ir::AtomKind::Number(n))) => {
    ...
    match s {
        ...
        Shape::App(op, args)
            if vm.session.operators.name(op) == Some("Slot")
                && (args.is_empty()
                    || (args.len() == 1
                        && matches!(vm.session.arena.get(args[0]), Some(athena_ir::TermKind::Atom(athena_ir::AtomKind::Number(n))) if n.as_exact_integer() == Some(1)))) =>
        {
            let _ = args;
            value
        }
