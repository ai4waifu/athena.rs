...
use super::{Outcome, arith::number_of, builtin::invalid_index_diagnostic, vm::Vm};
...
pub(crate) fn h_span(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    ...
    for a in args {
        match number_of(vm, *a).and_then(|n| n.as_exact_integer()) {
            Some(v) => ints.push(v),
            None => return Outcome::unevaluated(vm.push_app("Span", args.to_vec())),
        }
    }
    ...
    match list {
        ...
        None => Outcome::unevaluated(vm.push_app("Span", args.to_vec())),
    }
}
...
fn part_n(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    ...
    if is_all_symbol(vm, args[1]) && args.len() >= 3 && matches!(vm.session.arena.get(args[0]), Some(TermKind::List(_))) {
    ...
    Outcome { term: cur, kind: super::EvalKind::Value, status: athena_types::ComputationStatus::Exact, diagnostics: diags }
}

fn is_end_symbol(vm: &Vm<'_>, term: TermId) -> bool {
    match vm.session.arena.get(term) {
        Some(TermKind::Atom(AtomKind::Symbol(s))) => {
...
fn part_outcome(vm: &mut Vm<'_>, expr: TermId, index: TermId) -> Outcome {
    // 索引列表：逐个抽取再组列表。
    if matches!(vm.session.arena.get(index), Some(TermKind::List(_))) {
    ...
    if matches!(vm.session.arena.get(expr), Some(TermKind::List(_))) {
        ...
        let Some(idx) = number_of(vm, index).and_then(|n| n.as_exact_integer())
            ...
            return Outcome::unevaluated(vm.push_app("Part", vec![expr, index]));
        };
        if idx == 0 {
        ...
        let i = if idx > 0 {
            ...
            if pos < 0 || pos as usize >= len {
                let echo = vm.push_app("Part", vec![expr, index]);
...
pub(crate) fn h_apply(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    if matches!(vm.session.arena.get(args[1]), Some(TermKind::List(_))) {
...
pub(crate) fn h_replace_all(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    let rule_list: Vec<(TermId, TermId)> = match vm.session.arena.get(args[1]) {
        Some(TermKind::List(items)) => items.iter().filter_map(|r| rule_pair(vm, *r)).collect(),
        _ => rule_pair(vm, args[1]).into_iter().collect(),
    };
    let mut cur = args[0];
    for (lhs, rhs) in rule_list {
        cur = super::pattern::replace_literal(vm, cur, lhs, rhs);
    }
...
pub(crate) fn h_map(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    let Some(items) = (match vm.session.arena.get(args[1]) {
        Some(TermKind::List(_)) => vm.app_args(args[1]),
        _ => None,
    })
    else {
        return Outcome::value(vm.push_app("Map", vec![args[0], args[1]]));
    };
...
fn map_one(vm: &mut Vm<'_>, func: TermId, item: TermId) -> TermId {
    ...
    if is_function_arity(vm, func, 1) {
        let body = vm.app_args(func).unwrap_or_default()[0];
        return super::pattern::substitute_slot(vm, body, item);
    }
    if is_function_arity(vm, func, 2) {
        let fargs = vm.app_args(func).unwrap_or_default();
        if let Some(var) = symbol_name(vm, fargs[0]) {
            let sym = vm.session.arena.symbols_mut().intern(var);
            return super::pattern::substitute_symbol(vm, fargs[1], sym, item);
        }
    }
...
fn is_function_arity(vm: &Vm<'_>, id: TermId, arity: usize) -> bool {
    ...
    vm.app_args(id).is_some_and(|a| a.len() == arity)
}
