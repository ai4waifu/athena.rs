...
use crate::{
    arena_ops::{push_app_named, push_bool, push_int, push_list, push_null, push_symbol_name},
    session::Session,
};

use super::{
    EvalKind, Outcome,
    env::{DefinitionLayer, LocalBinding, ScopeFrame},
    kernel::ExecUnit,
    lower, rewrite,
};
...
impl UnitCache {
    ...
    fn lookup(&self, hash: u64, root: TermId, arena: &athena_ir::TermArena, mode: CompileMode) -> Option<Rc<ExecUnit>> {
...
impl<'a> Vm<'a> {
    ...
    pub(crate) fn rebuild_app_op(&mut self, op: OperatorId, args: Vec<TermId>) -> TermId {
        ...
        self.session.arena.push(TermKind::App { op, args }, span)
    }
    ...
    pub(crate) fn head_name(&self, id: TermId) -> Option<String> {
        match self.session.arena.get(id)? {
            TermKind::App { op, .. } => self.session.operators.name(*op).map(str::to_string),
            TermKind::List(_) => Some("List".into()),
            TermKind::Atom(AtomKind::Symbol(sym)) => self.session.arena.symbols().resolve(*sym).map(str::to_string),
            _ => None,
        }
    }
    ...
    pub(crate) fn app_args(&self, id: TermId) -> Option<Vec<TermId>> {
        match self.session.arena.get(id)? {
            ...
            TermKind::List(items) => Some(items.clone()),
            _ => None,
        }
    }
    ...
    pub(crate) fn shape(&self, id: TermId) -> Option<Shape> {
        match self.session.arena.get(id)? {
            TermKind::Atom(AtomKind::Number(_)) => Some(Shape::Number),
            TermKind::Atom(AtomKind::String(s)) => Some(Shape::Str(s.clone())),
            TermKind::Atom(AtomKind::Symbol(s)) => Some(Shape::Sym(*s)),
            TermKind::Atom(AtomKind::Boolean(b)) => Some(Shape::Bool(*b)),
            TermKind::Atom(AtomKind::Null) => Some(Shape::Null),
            TermKind::List(items) => Some(Shape::List(items.clone())),
            TermKind::App { op, args } => Some(Shape::App(*op, args.clone())),
        }
    }
    ...
    fn get_or_compile(&mut self, root: TermId, mode: CompileMode) -> Rc<ExecUnit> {
        ...
        let unit = lower::lower(self, root, mode);
        let rc = Rc::new(unit);
