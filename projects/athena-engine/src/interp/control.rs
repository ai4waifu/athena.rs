...
use super::{
    Outcome,
    builtin::{as_boolean_id, non_boolean_condition_diagnostic, term_summary},
    env::{LocalBinding, ScopeFrame},
    vm::Vm,
};
...
fn compound_into(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    ...
    Outcome { term: last, kind: super::EvalKind::Value, status: athena_types::ComputationStatus::Exact, diagnostics: diags }
}
...
fn while_loop(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    ...
    Outcome { term, kind: super::EvalKind::Unevaluated, status: athena_types::ComputationStatus::Invalid, diagnostics: diags }
}
...
fn for_loop(vm: &mut Vm<'_>, args: &[TermId]) -> Outcome {
    ...
    let var_sym = match vm.session.arena.get(args[0]) {
        Some(TermKind::Atom(AtomKind::Symbol(s))) => *s,
    ...
    let Some(values) = (match vm.session.arena.get(iter_o.term) {
        Some(TermKind::List(_)) => vm.app_args(iter_o.term),
        _ => None,
    })
    else {
    ...
    let mut last = vm.push_null();
    for value in values {
        let body = super::pattern::substitute_symbol(vm, args[2], var_sym, value);
...
fn with_block(vm: &mut Vm<'_>, args: &[TermId], top: bool, head: &str) -> Outcome {
    ...
    let Some(locals) = (match vm.session.arena.get(args[0]) {
        Some(TermKind::List(_)) => vm.app_args(args[0]),
        _ => None,
    })
    else {
...
fn match_set(vm: &Vm<'_>, term: TermId) -> Option<(athena_types::SymbolId, TermId)> {
    let TermKind::App { args, .. } = vm.session.arena.get(term)?
    else {
        return None;
    };
...
fn module_local(vm: &Vm<'_>, term: TermId) -> Option<(athena_types::SymbolId, Option<TermId>)> {
    ...
    match vm.session.arena.get(term) {
        Some(TermKind::Atom(AtomKind::Symbol(s))) => Some((*s, None)),
        _ => None,
    }
}
...
pub(crate) fn apply_function(vm: &mut Vm<'_>, fargs: &[TermId], args: &[TermId]) -> Outcome {
    match fargs {
        [body] if args.len() == 1 => {
            let substituted = super::pattern::substitute_slot(vm, *body, args[0]);
        ...
        [var, body] if args.len() == 1 => {
            if let Some(sym) = match vm.session.arena.get(*var) {
                ...
                let substituted = super::pattern::substitute_symbol(vm, *body, sym, args[0]);
                vm.eval_value(substituted)
            }
            else {
