...
pub(crate) fn solve(vm: &mut Vm<'_>, equation: ExprId, unknown: ExprId) -> Outcome {
    ...
    roots.sort_by(|a, b| {
        match (
            crate::execution::builtins::arithmetic::num_compare_ids(vm, *a, *b),
            crate::execution::builtins::arithmetic::number_of(vm, *a),
            crate::execution::builtins::arithmetic::number_of(vm, *b),
        ) {
            (Some(ord), _, _) => ord,
...
fn push_number_for_solve(vm: &mut Vm<'_>, n: &Number) -> ExprId {
    ...
    if let Some(r) = n.as_rational() {
        return super::matrix::rational_to_term(vm, r);
    }
    crate::execution::builtins::arithmetic::push_number(vm, clone_number(n))
}
    ...
    fn go(vm: &Vm<'_>, expr: ExprId, var: &str) -> Option<Vec<(Number, u32)>> {
        ...
        match vm.head_name(expr).as_deref() {
            ...
            Some("Power") => {
                ...
                let exp = crate::execution::builtins::arithmetic::number_of(vm, args[1])?.as_integer_exp()?;
                if exp < 0 {
                    return None;
                }
                let exp = exp as u32;
