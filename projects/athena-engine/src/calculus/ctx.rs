...
//!
//! 领域算法不再持有 owning 树：全部读写都落在调用方的 session `TermArena`，
...
use crate::{interp, interp::vm::Shape, numeric_clone::clone_number, session::Session};
...
impl<'a> CalculusCtx<'a> {
    ...
    pub(crate) fn shape(&self, id: TermId) -> Option<Shape> {
        match self.session().arena.get(id)? {
            athena_ir::TermKind::Atom(athena_ir::AtomKind::Number(_)) => Some(Shape::Number),
            athena_ir::TermKind::Atom(athena_ir::AtomKind::String(v)) => Some(Shape::Str(v.clone())),
            athena_ir::TermKind::Atom(athena_ir::AtomKind::Symbol(s)) => Some(Shape::Sym(*s)),
            athena_ir::TermKind::Atom(athena_ir::AtomKind::Boolean(b)) => Some(Shape::Bool(*b)),
            athena_ir::TermKind::Atom(athena_ir::AtomKind::Null) => Some(Shape::Null),
            athena_ir::TermKind::List(items) => Some(Shape::List(items.clone())),
            athena_ir::TermKind::App { op, args } => Some(Shape::App(*op, args.clone())),
        }
    }
    ...
    pub(crate) fn eval(&self, id: TermId) -> TermId {
        interp::vm::evaluate_session(self.session_mut(), id).term
    }
    ...
    pub(crate) fn num(&self, n: Number) -> TermId {
        interp::push_number(self.session_mut(), n)
    }

    /// 小型精确整数。
    pub(crate) fn in_(&self, n: i64) -> TermId {
        crate::arena_ops::push_int(self.session_mut(), n)
    }

    /// 机器实数。
    pub(crate) fn real(&self, x: f64) -> TermId {
        interp::push_number(self.session_mut(), Number::machine(x))
    }

    /// 符号原子。
    pub(crate) fn sym(&self, name: &str) -> TermId {
        crate::arena_ops::push_symbol_name(self.session_mut(), name)
    }
    ...
    pub(crate) fn list(&self, items: Vec<TermId>) -> TermId {
        crate::arena_ops::push_list(self.session_mut(), items)
    }

    /// 算子应用。
    pub(crate) fn ap(&self, head: &str, args: Vec<TermId>) -> TermId {
        crate::arena_ops::push_app_named(self.session_mut(), head, args)
    }
}
