...
use athena_engine::{
    CoefficientDomain, FieldId, Integer, MonomialOrder, RingTable, SymbolId,
};
...
fn finite_field_domain_is_field_id_only() {
    ...
    assert!(matches!(
        rings.coeff_rings().get(coeff_id).unwrap().domain,
        CoefficientDomain::FiniteField { field: _ }
    ));
}
...
fn unregistered_field_id_rejected_without_characteristic_in_domain() {
    let mut rings = RingTable::new();
    let err = rings
        .intern(CoefficientDomain::FiniteField { field: FieldId(42) }, vec![SymbolId(0)], MonomialOrder::Lex)
        .unwrap_err();
...
fn same_field_id_reuses_coeff_ring_regardless_of_session_order() {
    ...
    assert_eq!(
        rings.get(r1).unwrap().coefficient_ring,
        rings.get(r2).unwrap().coefficient_ring
    );
}
