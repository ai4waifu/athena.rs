...
use athena_engine::{
    bind_algorithm_checkpoint, resume_from_algorithm_checkpoint, GraphResidencyController, Session,
};
...
fn session_finish_graph_registers_object_roots() {
    ...
    assert!(session
        .heap()
        .borrow()
        .object_payload(published.snapshot_id().as_object())
        .is_ok());
}
...
fn lru_spills_unpinned_resident_chunks() {
    ...
    let (_published, csr) = tiny_directed()
        .finish_csr_on_heap(&mut h, &mut registry, budget)
        .unwrap();
...
fn checkpoint_resume_rejects_wrong_chunk_identity() {
    ...
    let mut registry = ChunkRegistry::new();
    let budget = MemoryBudget::new(64 * 1024).unwrap();
    let (published, _csr) = tiny_directed()
        .finish_csr_on_heap(&mut h, &mut registry, budget)
        .unwrap();
    let (mut checkpoint, _) = bind_algorithm_checkpoint(
        ...
        FrontierCheckpoint {
            queue: vec![NodeId(0)],
            discovered: vec![true, false, false],
            visited_prefix: vec![],
        },
    )
    .unwrap();
    checkpoint.revision = GraphRevision(99);
    let err = resume_from_algorithm_checkpoint(
        published.graph.as_graph(),
        &published.publication,
        checkpoint,
        None,
    )
...
fn checkpoint_resume_does_not_need_raw_pointers() {
    ...
    let DeterministicBfsOutcome::Cancelled { checkpoint: frontier, .. } = first else {
        panic!("expected cancelled");
    };
    let (checkpoint, _) = bind_algorithm_checkpoint(&published.publication, &mut h, frontier).unwrap();
    let resumed = resume_from_algorithm_checkpoint(published.graph.as_graph(), &published.publication, checkpoint, None)
        .unwrap();
    let DeterministicBfsOutcome::Complete(order) = resumed else {
