...
fn branch_module(cond: bool) -> ExecutionModule {
    ...
    let entry = BasicBlock {
        ...
        terminator: Terminator::Branch { condition: c, then_edge: BlockEdge::jump(BlockId(1)), else_edge: BlockEdge::jump(BlockId(2)) },
    };
    ...
    let mut module = ExecutionModule {
        inputs: Vec::new(),
        constants: vec![ConstantValue::boolean(cond), ConstantValue::boolean(true), ConstantValue::boolean(false)],
        captured_roots: Vec::new(),
        regions: vec![region],
        effect_edges: Vec::new(),
        exits: Vec::new(),
...
fn lower_and_execute_not_on_vm() {
    ...
    let value = execute_verified_cfg_on_vm(&mut session, &module, None).expect("vm").value;
...
fn lower_and_execute_boolean_branch_on_vm() {
    ...
    assert_eq!(execute_verified_cfg_on_vm(&mut session, &then_mod, None).expect("then").value, SlotValue::Boolean(true));
    let else_mod = branch_module(false);
    assert_eq!(execute_verified_cfg_on_vm(&mut session, &else_mod, None).expect("else").value, SlotValue::Boolean(false));
}
...
fn edge_arg_phi_module(cond: bool, then_val: bool, else_val: bool) -> ExecutionModule {
    ...
    let entry = BasicBlock {
        ...
        terminator: Terminator::Branch {
            condition: c,
            then_edge: BlockEdge { target: BlockId(1), arguments: vec![then_v] },
            else_edge: BlockEdge { target: BlockId(1), arguments: vec![else_v] },
        },
    };
    let join = BasicBlock {
        id: BlockId(1),
        parameters: vec![athena_engine::execution::ir::BlockParameter { value: param, ty: ExecutionValueType::Boolean }],
        operations: Vec::new(),
        terminator: Terminator::return_value(param),
    };
    let region = Region { id: RegionId(0), entry: BlockId(0), blocks: vec![entry, join], result_types: vec![ExecutionValueType::Boolean] };
    let mut module = ExecutionModule {
        inputs: Vec::new(),
        constants: vec![ConstantValue::boolean(cond), ConstantValue::boolean(then_val), ConstantValue::boolean(else_val)],
        captured_roots: Vec::new(),
        regions: vec![region],
        effect_edges: Vec::new(),
        exits: Vec::new(),
...
fn lower_guard_reject_passes_on_true() {
    ...
    assert_eq!(execute_verified_cfg_on_vm(&mut session, &module, None).expect("pass").value, SlotValue::Boolean(true));
}
...
fn lower_guard_reject_fails_on_false() {
    ...
    assert_eq!(err.details.get("reason").map(|v| v.to_string()).as_deref(), Some("rejected"));
}
...
fn lower_terminator_reject_on_else_edge() {
    ...
    let else_block =
        BasicBlock { id: BlockId(2), parameters: Vec::new(), operations: Vec::new(), terminator: Terminator::Reject { exit: None } };
    let region = Region {
        id: RegionId(0),
        entry: BlockId(0),
        blocks: vec![entry, then_block, else_block],
...
fn interfering_swap_phi_module() -> ExecutionModule {
    ...
    let entry = BasicBlock {
        ...
        terminator: Terminator::Branch {
            condition: v_true,
            then_edge: BlockEdge { target: BlockId(1), arguments: vec![v_false, v_true] },
            else_edge: BlockEdge { target: BlockId(1), arguments: vec![v_false, v_true] },
        },
    };
    let loop_block = BasicBlock {
        id: BlockId(1),
        parameters: vec![
            athena_engine::execution::ir::BlockParameter { value: p0, ty: ExecutionValueType::Boolean },
            athena_engine::execution::ir::BlockParameter { value: p1, ty: ExecutionValueType::Boolean },
        ],
        operations: Vec::new(),
        terminator: Terminator::Branch {
            condition: p0,
            then_edge: BlockEdge { target: BlockId(2), arguments: vec![p1] },
            else_edge: BlockEdge { target: BlockId(1), arguments: vec![p1, p0] },
        },
    };
    let exit = BasicBlock {
        id: BlockId(2),
        parameters: vec![athena_engine::execution::ir::BlockParameter { value: ret, ty: ExecutionValueType::Boolean }],
        operations: Vec::new(),
        terminator: Terminator::return_value(ret),
    };
    let region =
        Region { id: RegionId(0), entry: BlockId(0), blocks: vec![entry, loop_block, exit], result_types: vec![ExecutionValueType::Boolean] };
    let mut module = ExecutionModule {
        inputs: Vec::new(),
        constants: vec![ConstantValue::boolean(false), ConstantValue::boolean(true)],
        captured_roots: Vec::new(),
...
fn lower_interfering_edge_arg_swap_uses_temps() {
    ...
    let lowered = try_lower_verified_cfg_module(&module).expect("lower");
    let has_temp_move = lowered.module.instructions.iter().any(|insn| matches!(insn, athena_vm::Instruction::Move { dst, .. } if *dst >= 5));
    assert!(has_temp_move, "expected temporary Move slots for interfering phi");
    assert_eq!(execute_verified_cfg_on_vm(&mut session, &module, None).expect("swap").value, SlotValue::Boolean(false));
}
...
fn lower_and_execute_load_term_atom_on_vm() {
    ...
    assert!(try_lower_verified_cfg_module(&module).is_ok());
    assert_eq!(execute_verified_cfg_on_vm(&mut session, &module, None).expect("vm").value, SlotValue::Term(term));
}
