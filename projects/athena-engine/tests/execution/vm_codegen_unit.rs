...
fn lower_and_execute_boolean_branch_on_vm() {
    ...
    assert_eq!(
        execute_verified_cfg_on_vm(&mut session, &then_mod, None).expect("then").value,
        SlotValue::Boolean(true)
    );
    let else_mod = branch_module(false);
    assert_eq!(
        execute_verified_cfg_on_vm(&mut session, &else_mod, None).expect("else").value,
...
fn lower_guard_reject_passes_on_true() {
    ...
    assert_eq!(
        execute_verified_cfg_on_vm(&mut session, &module, None).expect("pass").value,
        SlotValue::Boolean(true)
    );
}
...
fn lower_interfering_edge_arg_swap_uses_temps() {
    ...
    assert_eq!(
        execute_verified_cfg_on_vm(&mut session, &module, None).expect("swap").value,
        SlotValue::Boolean(false)
    );
}
...
fn lower_and_execute_load_term_atom_on_vm() {
    ...
    assert!(try_lower_verified_cfg_module(&module).is_ok());
    assert_eq!(
        execute_verified_cfg_on_vm(&mut session, &module, None).expect("vm").value,
        SlotValue::Term(term)
    );
}
