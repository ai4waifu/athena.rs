...
fn and_of_integers_selects_athena_vm_after_host_truthiness() {
    ...
    let term = session.builder().application(ApplicationHead::Semantic(SemanticOperator::And), vec![a, b], Default::default());
...
fn sum_over_iterator_selects_athena_vm_after_host_fold() {
    ...
    let iter = session.builder().collection(athena_types::CollectionKind::OrderedCollection, vec![k, one, four], Default::default());
    let body = session.builder().application(ApplicationHead::Semantic(SemanticOperator::Power), vec![k, two], Default::default());
    let term = session.builder().application(ApplicationHead::Semantic(SemanticOperator::Sum), vec![body, iter], Default::default());
    let module = ExecutionCompiler::new().compile(&mut session, &AthenaRequest::Term(term)).expect("compile");
    let report = analyze_vm_capability(&module);
    assert!(report.supports_athena_vm, "gaps={:?}", report.gaps);
    assert_eq!(select_execution_backend(&module, false), BackendKind::AthenaVm);
...
fn differentiate_residual_selects_athena_vm() {
    ...
    let term = session.builder().application(ApplicationHead::Semantic(SemanticOperator::Differentiate), vec![x], Default::default());
    let module = ExecutionCompiler::new().compile(&mut session, &AthenaRequest::Term(term)).expect("compile");
    let report = analyze_vm_capability(&module);
    assert!(report.supports_athena_vm, "gaps={:?}", report.gaps);
    assert_eq!(select_execution_backend(&module, false), BackendKind::AthenaVm);
