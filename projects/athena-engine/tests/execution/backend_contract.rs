...
fn unwired_backends_never_silently_fallback() {
    ...
    let cases: [(&dyn ExecutionBackend, &str); 3] =
        [(&NativeJitBackend {}, "NativeJitBackend"), (&WasmBackend {}, "WasmBackend"), (&DomainKernelBackend {}, "DomainKernelBackend")];
    for (backend, component) in cases {
        ...
        assert_ne!(
            BackendAbiFingerprint::of_module(&module, backend.kind()),
            BackendAbiFingerprint::of_module(&module, BackendKind::Reference)
        );
    }
}
