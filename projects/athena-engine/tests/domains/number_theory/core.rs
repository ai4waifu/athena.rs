...
fn primality_distinguishes_probable() {
    ...
    let out = expect_nt(engine.execute_domain(
        &mut session,
        DomainRequest::NumberTheory(NumberTheoryRequest::PrimalityTest { n: 97.into(), miller_rabin_rounds: None }),
    ));
    match out {
    ...
    let out = expect_nt(engine.execute_domain(
        &mut session,
        DomainRequest::NumberTheory(NumberTheoryRequest::PrimalityTest { n: 91.into(), miller_rabin_rounds: None }),
    ));
    match out {
...
fn factor_integer_complete_small() {
    ...
    let out = expect_nt(engine.execute_domain(
        &mut session,
        DomainRequest::NumberTheory(NumberTheoryRequest::FactorInteger { n: (-360).into(), limits: FactorLimits::default() }),
    ));
    match out {
...
fn modular_inverse_and_pow() {
    ...
    let out = expect_nt(engine.execute_domain(
        &mut session,
        DomainRequest::NumberTheory(NumberTheoryRequest::ModInverse { a: 3.into(), modulus: clone_modulus(&m) }),
    ));
    match out {
...
fn factor_zero_invalid_input() {
    ...
    let out = expect_nt(engine.execute_domain(
        &mut session,
        DomainRequest::NumberTheory(NumberTheoryRequest::FactorInteger { n: Integer::zero(), limits: FactorLimits::default() }),
    ));
    match out {
