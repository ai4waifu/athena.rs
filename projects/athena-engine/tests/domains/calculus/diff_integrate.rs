...
fn special_function_registry_derivatives() {
    ...
    assert!(athena_engine::execution::builtins::registry::lookup_function("Erf").is_some());
    assert!(athena_engine::execution::builtins::registry::registered_function_names().any(|n| n == "Gamma"));
}
...
fn curl_of_linear_3d_field() {
    ...
    match out {
        CalculusResult::Exact { value: CalculusValue::Curl(c), .. } => {
            assert_eq!(c.curl_components.iter().map(|t| h.dbg(*t)).collect::<Vec<_>>(), vec!["0", "0", "2"]);
        }
...
fn fourier_exp_abs_decay() {
    ...
    let out = run(
        ...
        &h,
        CalculusRequest::Transform {
            kind: athena_engine::domains::calculus::TransformKind::Fourier,
            expression: expr,
            time_variable: "t".into(),
    ...
    match out {
        CalculusResult::Exact { value: CalculusValue::Transform(tr), .. } => {
            assert!(tr.region_of_convergence.known);
            assert_eq!(tr.kind, athena_engine::domains::calculus::TransformKind::Fourier);
            let text = h.dbg(tr.expression);
...
fn z_transform_geometric_and_delta() {
    ...
    let out = run(
        ...
        &h,
        CalculusRequest::Transform {
            kind: athena_engine::domains::calculus::TransformKind::Z,
            expression: geom,
    ...
    match out {
        CalculusResult::Exact { value: CalculusValue::Transform(tr), .. } => {
            assert!(tr.region_of_convergence.known);
            assert_eq!(tr.kind, athena_engine::domains::calculus::TransformKind::Z);
            let text = h.dbg(tr.expression);
            assert!(text.contains('z'), "got {text}");
            let roc = tr.region_of_convergence.predicate.map(|p| h.dbg(p)).unwrap_or_default();
