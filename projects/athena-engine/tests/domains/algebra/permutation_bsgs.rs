...
use athena_engine::domains::{
    algebra::GroupTable,
    group::{
        GroupElementRepr, GroupRequest, Permutation, canonical_permutation, execute_group_with_table_mut, group_membership,
        inverse_group_element, multiply_group_elements,
    },
};
...
fn execute_group_registers_permutation_group() {
    ...
    assert!(matches!(r, athena_engine::domains::group::GroupResult::Exact { .. }));
}
...
fn klein_four_is_abelian() {
    ...
    let r = athena_engine::domains::group::execute_group_with_table(GroupRequest::IsAbelian { group: g }, &table);
    match r {
        athena_engine::domains::group::GroupResult::Exact { value } => {
            assert!(matches!(value, athena_engine::domains::group::GroupDomainValue::Boolean(true)));
        }
...
fn symmetric_three_not_abelian() {
    ...
    match r {
        athena_engine::domains::group::GroupResult::Exact { value } => {
            assert!(matches!(value, athena_engine::domains::group::GroupDomainValue::Boolean(false)));
        }
        _ => panic!("expected boolean"),
    }
}
