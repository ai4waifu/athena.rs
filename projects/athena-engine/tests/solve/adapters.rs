...
use athena_types::{AssumptionSetId, OperatorId};
...
fn univariate_linear_root_complete() {
    ...
    let adapted =
        solve_univariate_polynomial_roots(p, &rings, unknown, SolveDomain::Rationals, PolynomialFactorLimits::default())
            .unwrap();
    assert_eq!(adapted.factorization_completeness, athena_engine::PolynomialFactorizationCompleteness::Complete);
    assert!(matches!(adapted.solution.coverage, CoverageStatus::Complete));
    assert_eq!(adapted.solution.branches.len(), 1);
...
fn normalize_equal_keeps_lhs_rhs() {
    ...
    match c {
        athena_engine::Constraint::Equation(e) => {
            assert_eq!(e.lhs, lhs);
            assert_eq!(e.rhs, rhs);
    ...
    assert!(matches!(arena.get(eq), Some(TermKind::App { args, .. }) if args.len() == 2));
}
