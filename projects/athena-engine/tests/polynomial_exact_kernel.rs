...
use athena_engine::{
    CoefficientDomain, Integer, MonomialOrder, Number, PolynomialBuilder, PolynomialRequest, PolynomialResult, RingTable,
    Session, SymbolId, add_polynomial, execute_polynomial, mul_polynomial, sub_polynomial,
};

fn z_x_ring() -> (RingTable, athena_engine::RingId) {
    let mut rings = RingTable::new();
    let id = rings.intern(CoefficientDomain::Integer, vec![SymbolId(0)], MonomialOrder::Lex).unwrap();
    (rings, id)
}

fn q_x_ring() -> (RingTable, athena_engine::RingId) {
    let mut rings = RingTable::new();
...
fn build_univariate(rings: &RingTable, ring: athena_engine::RingId, terms: &[(i64, u32)]) -> athena_engine::Polynomial {
