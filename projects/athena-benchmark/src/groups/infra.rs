...
use athena_graph::{bfs_order, CsrGraph, Graph, GraphDirection};
use athena_ndarray::{array1d, LogicalShape, MemoryBudget};
use athena_table::{column_from_store, Field, LogicalType, Schema, Table, TableExpr};
...
impl Fixture for NdarrayBudgetFixture {
    fn meta(&self) -> FixtureMeta {
        FixtureMeta {
            id: "infra.ndarray_chunk_budget",
            group: BenchGroup::Infra,
            scale: "len_10_budget_24",
            domain: "ndarray",
        }
    }

    fn validate(&self) -> Result<ValidationSummary, String> {
        let a = array1d((0u64..10).collect(), MemoryBudget::new(24).unwrap())
            .map_err(|e| format!("{e:?}"))?;
        let mut sizes = Vec::new();
        a.for_each_chunk(|_, c| sizes.push(c.len()))
            .map_err(|e| format!("{e:?}"))?;
        if sizes != [3, 3, 3, 1] {
...
impl Fixture for GraphCsrFixture {
    fn meta(&self) -> FixtureMeta {
        FixtureMeta {
            id: "infra.graph_csr_stream",
            group: BenchGroup::Infra,
            scale: "3_nodes_5_edges",
            domain: "graph",
        }
    }
    ...
    fn validate(&self) -> Result<ValidationSummary, String> {
        ...
        graph
            .for_each_neighbor_chunk(0, |c| chunks.push(c.to_vec()))
            .map_err(|e| format!("{e:?}"))?;
        if chunks.len() != 3 {
...
impl Fixture for TableLazyFixture {
    ...
    fn validate(&self) -> Result<ValidationSummary, String> {
        let schema = Schema::new([
            Field::new("id", LogicalType::Int(64), false),
