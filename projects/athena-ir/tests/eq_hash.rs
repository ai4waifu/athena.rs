use athena_ir::{AtomKind, OperatorRegistry, TermArena, TermBuilder, TermKind, canonical_hash, canonical_hash_named, fnv1a64};
use athena_types::SourceSpan;

const SPAN: SourceSpan = SourceSpan { start: 0, end: 0 };

fn build_plus_x_plus_y(x_first: bool) -> (TermArena, athena_types::TermId) {
...
fn structural_eq_atom_payloads() {
    ...
    assert!(matches!(arena.get(i1), Some(TermKind::Atom(AtomKind::Number(_)))));
}
