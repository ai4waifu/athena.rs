...
use athena_table::{
    column_from_store, Field, LogicalType, Schema, Table, TableExpr,
};
...
fn rejects_duplicate_fields() {
    let err = Schema::new([
        Field::new("a", LogicalType::Int(64), true),
        Field::new("a", LogicalType::Utf8, true),
    ]);
    assert!(err.is_err());
}
...
fn lazy_select_limit_preserves_schema() {
    ...
    .unwrap();
    let table = Table::with_rows(schema.clone(), 100)
        .lazy()
