...
fn register_compiled_rule_dispatches_extension_apply() {
    ...
    fx.execute_request(AthenaRequest::Command(SessionCommand::RegisterRuleDispatch { table, rule })).expect("register compiled rule");
...
fn semantic_head_is_not_extension_operator_id() {
    ...
    match fx.session().arena.get(add) {
        Some(TermNode::Application { head: ApplicationHead::Semantic(SemanticOperator::Add), .. }) => {}
    ...
    match fx.session().arena.get(call) {
        Some(TermNode::Application { head: ApplicationHead::Extension(id), .. }) => assert_eq!(*id, ext),
...
fn domain_goal_derivative_uses_typed_calculus_request() {
    ...
    let result = engine.execute_domain(fx.session_mut(), DomainRequest::Calculus(req)).expect("sin'");
    match result {
...
fn number_theory_goal_request_preserves_typed_payload() {
    let mut fx = SessionFixture::new();
    let goal =
        fx.domain().dispatch(DomainRequest::NumberTheory(NumberTheoryRequest::Gcd { a: Integer::from_i64(12), b: Integer::from_i64(8) }));
...
fn structural_term_equality_admits_into_exact_uf_and_proof_forest() {
    ...
    fx.session_mut().admit_structural_term_equality(left, right).expect("admit");
...
fn typed_egraph_saturation_binds_add_same() {
    use athena_engine::reasoning::{egraph::TypedRuleSet, trs::TermPattern};
...
fn egraph_extract_smallest_ast_after_typed_saturation() {
    use athena_engine::reasoning::{
        egraph::{ExtractionPreference, TypedRuleSet},
        trs::TermPattern,
    };
    use athena_ir::{ApplicationHead, SemanticOperator};
    ...
    let extracted = fx.session().extract_egraph_class(class, ExtractionPreference::SmallestAst).expect("extract");
...
fn egraph_extract_result_cost_prefers_admitted_rep() {
    use athena_engine::reasoning::{
        egraph::ExtractionPreference,
        mgraph::{AdmissionGate, Claim, Evidence, EvidenceCertificate, Guarantee, Proposition, Scope, VerificationPolicy},
    };
...
fn typed_egraph_admit_pipeline_replays_rewrite_rules() {
    ...
    let mut rules = TypedRuleSet::new();
    rules.push(pattern, x_term, Some("add_same"));
    let report = fx.session_mut().run_typed_egraph_admit_pipeline(&[add], Some(&rules), 8);
...
fn predicate_registry_and_hyper_edge_staging_are_typed() {
    use athena_engine::reasoning::mgraph::{
        CalculusRelationKind, Guarantee, HyperEdge, Proposition, arity_ok, descriptor, hyper_edge_to_outer_candidate, predicates,
    };
...
fn typed_rewrite_replay_admits_add_same_into_exact_uf() {
    ...
    let report = fx.session_mut().run_egraph_saturation_typed(&[add], Some(&rules));
    assert_eq!(report.candidates.len(), 1);
    let admitted = fx.session_mut().admit_typed_rewrite_egraph_candidates(&rules, &report.candidates);
    assert_eq!(admitted.len(), 1);
    assert!(admitted[0].is_ok());
    let derived = &fx.session().mgraph.semantic.derived;
    assert_eq!(derived.exact_uf.find(add), derived.exact_uf.find(one));
...
fn mgraph_closure_materializes_transitivity_in_proof_forest() {
    use athena_engine::reasoning::mgraph::{
        AdmissionGate, Claim, ClosureLimits, ClosureStopReason, Evidence, EvidenceCertificate, Guarantee, ProofStepKind, Proposition, Scope,
        VerificationPolicy,
    };
    ...
    assert!(
        fx.session()
            ...
            .edges()
            .iter()
            .any(|e| { e.step_kind == ProofStepKind::Transitivity && ((e.left == a && e.right == c) || (e.left == c && e.right == a)) })
    );
}
...
fn mgraph_closure_drains_hyper_edges_without_admit() {
    ...
    fx.session_mut().mgraph.operational.hyper_edges.push(HyperEdge { nodes: vec![left, right], predicate: predicates::REWRITE_EQUIVALENT });
    let result = fx.session_mut().run_mgraph_closure(ClosureLimits::default());
    ...
    assert_eq!(fx.session().mgraph.operational.outer_candidates[0].claim.guarantee, Guarantee::Candidate);
    assert_eq!(fx.session().mgraph.operational.outer_candidates[0].claim.proposition, Proposition::TermEquality { left, right });
...
fn mgraph_outer_pool_admits_only_structural_equality() {
    ...
    let (same_a, same_b, left, right) = {
        ...
        let same_a = session.arena.push(TermNode::Atom(Atom::Number(athena_numeric::Number::small_int(3))), span);
        let same_b = session.arena.push(TermNode::Atom(Atom::Number(athena_numeric::Number::small_int(3))), span);
    ...
    fx.session_mut().mgraph.operational.hyper_edges.push(HyperEdge { nodes: vec![same_a, same_b], predicate: predicates::REWRITE_EQUIVALENT });
    fx.session_mut().mgraph.operational.hyper_edges.push(HyperEdge { nodes: vec![left, right], predicate: predicates::REWRITE_EQUIVALENT });
...
fn mgraph_admit_wakes_registered_obligation() {
    use athena_engine::reasoning::mgraph::{
        Claim, Evidence, EvidenceCertificate, Guarantee, POLYNOMIAL_PROVIDER_ID, ProofObligation, Proposition, Scope, ScopeRef, predicates,
    };
...
fn mgraph_schedule_wakes_queues_need_computation() {
    use athena_engine::{
        ...
        reasoning::mgraph::{FactId, MGraphView, ProofObligation, Reflection, ReflectorWake, ScopeRef, SemanticReflector, predicates},
    };
    ...
    impl SemanticReflector for AlwaysCompute {
        fn reflect(&self, _obligation: &ProofObligation, _view: &MGraphView<'_>) -> Reflection {
            Reflection::NeedComputation { plan: DomainPlan { steps: vec![PlanStep::CallDomainProvider, PlanStep::MaterializeResult] } }
        }
    }

    let mut fx = SessionFixture::new();
    let wakes = [ReflectorWake {
        obligation: ProofObligation { predicate: predicates::POLYNOMIAL_RESULT, scope: ScopeRef::UNCONDITIONAL, known_objects: vec![] },
        relation: FactId(0),
    }];
    let report = fx.session_mut().schedule_mgraph_reflector_wakes(&wakes, &AlwaysCompute);
    assert_eq!(report.need_computation, 1);
    assert_eq!(fx.session().mgraph.operational.pending_plans.len(), 1);
    assert_eq!(fx.session().mgraph.operational.pending_plans[0].obligation.predicate, predicates::POLYNOMIAL_RESULT);
    assert!(matches!(fx.session().mgraph.operational.pending_plans[0].binding, athena_engine::reasoning::mgraph::PlanBinding::Unbound));
}
