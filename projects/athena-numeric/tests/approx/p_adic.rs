...
fn padic_add_mul_inv_mod_five() {
    ...
    let a = PAdicValue::from_integer(&2.into(), p.clone(), 4).unwrap();
    let b = PAdicValue::from_integer(&3.into(), p.clone(), 4).unwrap();
    let sum = a.add(&b).unwrap();
    assert_eq!(sum, PAdicValue::from_integer(&5.into(), p.clone(), 4).unwrap());
    assert!(!sum.is_zero());

    let zero = a.add(&PAdicValue::from_integer(&(-2).into(), p.clone(), 4).unwrap()).unwrap();
    assert!(zero.is_zero());

    let prod = a.mul(&b).unwrap();
    assert_eq!(prod, PAdicValue::from_integer(&6.into(), p.clone(), 4).unwrap());
...
fn padic_from_rational_and_validate() {
    ...
    let v = PAdicValue::from_rational(&r, p.clone(), 3).unwrap();
    assert!(v.is_unit());
    let nv = NumericValue::PAdic(v.clone());
    nv.validate().unwrap();
