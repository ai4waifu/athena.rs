...
use athena_numeric::{Integer, NumericContext, natural::Natural};
use std::str::FromStr;
...
fn natural_gcd_matches_euclidean_reference() {
    ...
    for _ in 0..64 {
        ...
        assert_eq!(
            a.gcd(&b),
            reference_gcd(
                a.try_clone_in(&NumericContext::portable_default()).unwrap(),
                b.try_clone_in(&NumericContext::portable_default()).unwrap()
            )
        );
    }
}
