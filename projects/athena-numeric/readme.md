...
- **promotion**：Integer↔Rational · Exact↔Machine · Machine↔Arbitrary · mismatch 诊断（见 `tests/exact/promotion.rs`；CI：
  `cargo test -p athena-numeric --test main promotion`）

`num-*` 不出现在 workspace 根依赖；核心 crate 零 `num-*`。可选外部对照仅见 `athena-benchmark`（`compare-*` feature）。依赖：
`athena-types`。
