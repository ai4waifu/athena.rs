...
//!
//! [`TemporaryNatural`] / [`TemporaryInteger`] 是与持久 [`super::natural::Natural`] /
...
/// 批内临时非负幅度（生命周期绑在 [`NumericBatch`] 独占借用上）。
///
...
/// 批内临时有符号整数（同 [`TemporaryNatural`] 生命周期合同）。
///
/// Heap 模式结果位于 ephemeral bump；不得逃逸 batch，除非 [`Self::promote`]。
