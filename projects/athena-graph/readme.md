...
```text
identity/     GraphId · GraphRevision · GraphSnapshot · GraphSemantics · Graph
storage/      CSR/CSC · property · capability · conversion
views/        reversed · induced · edge-filtered + ViewMapping
...
- **身份**：`GraphId` · `GraphRevision` · `NodeRef`/`EdgeRef`（绑 revision）· `GraphSnapshot`
- **表示**：内存邻接表 · `CsrGraph` · `CscGraph`
- **视图**：`ReversedGraphView` · `InducedSubgraphView` · `EdgeFilteredView`
