//! 类型化 property / weight 列。

use athena_graph::{GraphError, PropertyCell, PropertyColumn, PropertyStore, WeightColumn, WeightDomainTag};
...
fn property_store_binds_node_and_edge_columns() {
    ...
    store.insert_node_column("label", 2, PropertyColumn::try_from_dense(2, vec!["a", "b"]).unwrap()).unwrap();
    store.insert_edge_column("cap", 1, PropertyColumn::try_from_cells(1, vec![PropertyCell::Present(7)]).unwrap()).unwrap();
...
fn property_store_rejects_edge_column_wrong_len() {
    ...
    let err = store.insert_edge_column("w", 2, PropertyColumn::try_from_dense(1, vec![1]).unwrap()).unwrap_err();
...
fn weight_column_rejects_mismatch() {
    let err = WeightColumn::try_new(WeightDomainTag::MachineReal, 2, vec![PropertyCell::Present(1.0f64)]).unwrap_err();
    assert_eq!(err, GraphError::PropertyLengthMismatch { expected: 2, actual: 1 });
}
