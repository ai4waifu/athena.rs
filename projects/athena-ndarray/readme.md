...
身份合同：`ArrayId` ≠ `LogicalShape` ≠ `ArrayLayout` ≠ `MemoryBudget`；简版挂 `athena-gc`（`ArraySnapshotId` /
`ArrayChunkId` · `RootKind::Array`）。 与 `MatrixId`（engine 线代）及 `GraphChunkId`（图存储）分工，不混身份。
